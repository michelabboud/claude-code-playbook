...
6.2 **Every task ends with a close-out report**, five sections — the audit side of the autonomy contract (rule 7.2):
    ...
    2. **Verification evidence** — actual test/build/lint output (rule 2.2), measurements for any performance claim (rule 2.3); when subagents were used, a one-line model ledger (which tiers ran what, e.g. "12 impl on sonnet, 6 reviews on sonnet, 1 final review on opus") so the spend is auditable
