...
| 3.3 | **Pipelined, never fire-and-forget.** Review batch N while N+1 builds; every finding pins a commit and is re-checked against tip before anyone acts; a blocker **stops the line**. **A review's input is a commit, never a working tree:** the reviewer reads git objects only and builds in its own detached worktree; the brief defines what counts as blocking; the coordinator commits the reviewer's output, that one path only, the moment it is complete; in-process subagents do not count as blind. **Read the file before dispatching a reviewer.** |
...
| 3.5 | **How far development may run ahead of review.** Mechanical review never holds development. Deep review: at most **two unruled batches under a line's tip** — one is normal, the second is announced loudly, a third never starts — **counted by ancestry**, so a branch off unreviewed work inherits its count and a merge adds them. Three waits hold at any depth: work that is expensive to undo · anything irreversible or outward-facing · a blocker. **High deep reviews are gates**, the count drains to zero before one, and the wait works the queue of minor findings. The planner sequences independent work after every batch boundary. |
